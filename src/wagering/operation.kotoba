(ns wagering.operation
  "OperationActor -- one gambling/betting operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (WagerOps-LLM) is sealed into a single node (:advise); its proposal
  is ALWAYS routed through the Responsible Gambling Governor (:govern)
  ...
  One graph run = one gambling/betting operation (intake -> advise ->
  ...
  decision to a human operator (the licensed gaming supervisor). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:wager/accept`/`:payout/settle` ALWAYS reach this node
  when the governor is clean -- see `wagering.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [wagering.wageropsllm :as wageropsllm]
            [wagering.governor :as governor]
...
(defn build
  ...
  `wagering.store/Store`).
  opts:
    :advisor      -- a `wagering.wageropsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (wageropsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; WagerOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (wageropsllm/-advise advisor store request)]
            {:proposal p :audit [(wageropsllm/trace request p)]})))

      ;; Responsible Gambling Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
